...
//   for(auto node:data){
//     std::transform(node.second.name.begin(),node.second.name.end(),node.second.name.begin(),::tolower);
...
//     if(node_name.find(name) == 0) results.push_back(node.second.name);
//   }
//   return results;
// }
...
//     // std::reverse(shortest_path.begin(), shortest_path.end());
//     return shortest_path;
//   } else if (distance_B + distance_C < distance_A + distance_B && distance_B + distance_C < distance_A + distance_C)  {
...
std::vector<std::string> TrojanMap::TrojanPath(std::vector<std::string> &location_names) {
  ...
  // Calculate the shortest path that visits all locations
  std::vector<std::string> shortest_path;
  double shortest_distance = std::numeric_limits<double>::max();
  std::vector<std::string> permutation = location_names;
  do {
    std::vector<std::string> path;
    double distance = 0.0;
    for (int i = 0; i < permutation.size() - 1; i++) {
      std::vector<std::string> sub_path = CalculateShortestPath_Dijkstra(permutation[i], permutation[i+1]);
      path.insert(path.end(), sub_path.begin(), sub_path.end() - 1);
      distance += CalculatePathLength(sub_path);
    }
    path.push_back(permutation.back());
    distance += CalculatePathLength(CalculateShortestPath_Dijkstra(path.back(), permutation.front()));
